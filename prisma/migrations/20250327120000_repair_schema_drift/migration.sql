-- Repair: migration history can be out of sync with the actual database (e.g. missing "RideSession").
-- Idempotent: safe if objects already exist (IF NOT EXISTS / exception handlers).
...
CREATE TABLE IF NOT EXISTS "RideSession" (
    "id" TEXT NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS "Group_telegramChatId_key" ON "Group"("telegramChatId");
CREATE INDEX IF NOT EXISTS "RideSession_userId_status_idx" ON "RideSession"("userId", "status");
CREATE INDEX IF NOT EXISTS "RideSession_groupId_status_idx" ON "RideSession"("groupId", "status");
...
CREATE UNIQUE INDEX IF NOT EXISTS "LocationCache_latRounded_lngRounded_key" ON "LocationCache"("latRounded", "lngRounded");
...
CREATE UNIQUE INDEX IF NOT EXISTS "RideSession_one_active_per_user_idx" ON "RideSession" ("userId") WHERE "status" = 'ACTIVE';
